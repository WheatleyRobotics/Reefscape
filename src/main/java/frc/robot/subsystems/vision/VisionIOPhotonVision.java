...
public class VisionIOPhotonVision implements VisionIO {
  ...
  private final PhotonPoseEstimator trigEstimator;
  private final PhotonPoseEstimator singleEstimator;
  ...
  public VisionIOPhotonVision(
    ...
    singleEstimator =
        new PhotonPoseEstimator(
            aprilTagLayout, PhotonPoseEstimator.PoseStrategy.LOWEST_AMBIGUITY, robotToCamera);
  }
  ...
  public void updateInputs(VisionIOInputs inputs) {
    ...
    for (var result : camera.getAllUnreadResults()) {
      ...
      singleEstimator.addHeadingData(
          result.getTimestampSeconds(), new Rotation3d(rotationSupplier.get()));
      trigEstimator.addHeadingData(
          result.getTimestampSeconds(), new Rotation3d(rotationSupplier.get()));
      // Add pose observation
      if (result.hasTargets()) {
        if (shouldTrigSolve) {
          var estimatedRobotPose = trigEstimator.update(result).get();
